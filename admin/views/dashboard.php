...
<div class="wrap">
	...
	<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="max-width:720px;margin-bottom:1.5em;padding:12px 12px 4px;border:1px solid #c3c4c7;background:#fff;">
		...
		<?php submit_button( __( 'Save settings', 'reactwoo-geo-commerce' ), 'secondary', 'submit', false ); ?>
	</form>

	<p><?php esc_html_e( 'WooCommerce-aware personalization extends Geo Core visitor data. Configure country-based unit price rules under Commerce pricing.', 'reactwoo-geo-commerce' ); ?></p>
	<p>
	...
	<?php if ( ! empty( $sample ) && is_array( $sample ) ) : ?>
		<pre style="max-width:720px;overflow:auto"><?php echo esc_html( wp_json_encode( $sample, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) ); ?></pre>
	<?php else : ?>
